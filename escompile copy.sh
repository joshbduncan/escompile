...
process_import() {
    ...
    if [[ ! -f "$FILE" ]]; then
        ...
        for INCLUDE_PATH in "${INCLUDE_PATHS[@]-}"; do
            ...
            if [[ -f "$INCLUDE_PATH/$FILE" ]]; then
                FILE="$INCLUDE_PATH/$FILE"
                break
            fi
        done
        if [[ ! -f "$FILE" ]]; then
            echo >&2 "[error]: '$FILE': No such file."
            exit 1
        fi
    fi
    ...
    while IFS= read -r LINE || [[ -n "$LINE" ]]; do
        ...
        # extract any `includepath` statements and add them to the `INCLUDE_PATHS` array
        if [[ $TRIMMED =~ (\#|//\@)includepath ]]; then
            # extract just the value
            VALUE=$(sed -n -e 's/^.*includepath[[:blank:]]//p' <<<"$TRIMMED" | tr -d \"\')

            # save reference to current $IFS, set $IFS to specified separater ";"
            OLDIFS=$IFS
            IFS=";"

            # split value by ";" incase multiple paths were specified
            for INCLUDE_PATH in $VALUE; do
        ...
        # recursivly follow any include statements
        if [[ "$TRIMMED" =~ (\#|//\@)include ]]; then

            # get the leading whitespace for the include line
            CURRENT_WS=$((${#LINE} - ${#TRIMMED} + WS))

            # extract just the path value
            FPATH=$(sed -n -e 's/^.*include[[:blank:]]//p' <<<"$TRIMMED" | tr -d \"\')

            # check if an absolute path was specified
            if [[ "$FPATH" == /* ]]; then
                IMPORT_PATH="$FPATH"
            else
                ...
                for INCLUDE_PATH in "${INCLUDE_PATHS[@]-}"; do
                    # if file is found at current include path break
                    if [[ -f "$CUR_BASE_DIR/$INCLUDE_PATH/$FPATH" ]]; then
                        IMPORT_PATH="$CUR_BASE_DIR/$INCLUDE_PATH/$FPATH"
                        break
                    fi
                done
            fi
            ...
            process_import "$IMPORT_PATH" "$CURRENT_WS"
        else
            # escape line
            LINE_ESCAPED=$(printf '%s\n' "$LINE")

            # print the line as it is if no import statement is found
            echo "${PADDING}${LINE_ESCAPED}"
        fi
